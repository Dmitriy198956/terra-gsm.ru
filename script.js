(() => {
  ...
  // Model/brand tabs in the quick price guide.
  const brandTabs = [...document.querySelectorAll('[data-brand-tab]')];
  const repairRows = [...document.querySelectorAll('[data-brand-row]')];
  ...
  const switchBrand = (brand) => {
    ...
    repairRows.forEach((row) => {
      row.hidden = row.dataset.brandRow !== brand;
    });
  };

  brandTabs.forEach((tab) => {
    tab.addEventListener('click', () => switchBrand(tab.dataset.brandTab));
  });
